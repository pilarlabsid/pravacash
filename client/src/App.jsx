...
const LoadingOverlay = ({ message = "Memuat..." }) => {
  return (
...
function App() {
  ...
  const submitCreateUser = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        setToast({ type: "success", message: data.message || "User berhasil dibuat." });
    ...
    return (
      <>
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-indigo-900 to-slate-800 flex items-center justify-center px-4">
          {toast && (
            <div
              className={`fixed right-6 top-6 z-50 rounded-2xl px-4 py-3 text-white shadow-2xl transition-all ${toast.type === "error"
                ? "bg-rose-500/90"
                : "bg-emerald-500/90"
                }`}
            >
            ...
            <div className="space-y-6">
              ...
              <div className="flex gap-2 rounded-2xl bg-white p-2 shadow-soft">
                <button
                  ...
                  className={`flex-1 rounded-xl px-4 py-3 text-sm font-semibold transition ${adminTab === "dashboard"
                ...
                <button
                  ...
                  className={`flex-1 rounded-xl px-4 py-3 text-sm font-semibold transition ${adminTab === "users"
                    ? "bg-indigo-600 text-white"
                    : "text-slate-600 hover:bg-slate-50"
                    }`}
                >
                ...
                <button
                  ...
                  className={`flex-1 rounded-xl px-4 py-3 text-sm font-semibold transition ${adminTab === "transactions"
                    ? "bg-indigo-600 text-white"
                    : "text-slate-600 hover:bg-slate-50"
                    }`}
                >
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex justify-end">
                    <button
                      ...
                      className="inline-flex items-center gap-2 rounded-2xl bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white shadow-soft transition hover:bg-indigo-700"
                    >
                          ...
                          {adminUsers.map((u) => (
                            <tr key={u.id}>
                              ...
                              <td className="px-4 py-3">
                                <span
                                  className={`inline-flex rounded-full px-2 py-1 text-xs font-semibold ${u.role === "admin"
...
const Badge = ({ label, variant, size = "md" }) => {
  ...
  return (
    <span
      className={`inline-flex rounded-full font-semibold ${variants[variant] ?? variants.neutral
        } ${sizes[size] ?? sizes.md}`}
    >
      {label}
    </span>
